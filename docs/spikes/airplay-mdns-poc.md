...
3. `scripts/airplay-mdns-advertise.js` を `npm run advertise:airplay -- --ip <Windows LAN IP> --name AirLink-Phase0` で起動し、広告開始ログを保存する。
4. iPhone の画面ミラーリング候補に表示されるか確認する。
5. 候補に出ない場合は、service type / TXT レコード / Firewall / VPN / AP isolation の順に切り分ける。
6. 候補に出た場合も、映像受信ではなく接続試行ログの有無までを Phase 0 の確認範囲として記録する。
